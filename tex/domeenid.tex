...
\begin{align*} % nasty hack
	\tf_{2,3}(d) &= d[\texttt{x} \mapsto d(\texttt{x}) \cap \{0\}], &
...
\end{gather*}
kus kirjutis $d[\texttt{x} \mapsto v]$ tähendab funktsiooni uuendamist (\ingl{function update}) ehk funktsiooni, mis argumendil \texttt{x} omab väärtust $v$ ja ülejäänud argumentidel sama väärtust, mis funktsioon $d$. Aritmeetilised tehted hulkade vahel on sellised nagu nad on sisemises domeenis $\Val$.
...
\TODO{Üksik lause kuhugi mujale}

...
\vspace{-10ex} % hack to fit both on one page
\begin{center}
\noindent
\begin{tikzpicture}[
	level distance=1cm,
											...
											child { node {ProdSimple}
												child { node {ProdConf}
													...
													child [narrow] { node {Flat}
														child { node {Thread}
															child { node [bot] (BaseSouthEast) {(Variables)} }
														}
													}
												}
											}
								...
								child { node (EscapeNorth) {EscapedVars}
									child { node {ToppedSet}
										child { node {SetDomain}
											child { node [bot] (EscapeSouth) {(Variables)} }
										}
									}
								}
								% mutex
									...
									child { node {Reverse}
										child { node {ToppedSet}
											child { node {SetDomain}
												child { node {Prod}
													child { node {ProdConf}
														child [narrow] { node {NormalLat}
															child { node [bot] (MutexSouthWest) {(Normal)} }
														}
														child [narrow] { node (MutexEast) {Booleans} }
													}
												}
											}
										}
...
\node[group,fit=(BaseNorth) (BaseWest) (BaseSouthEast),label=below:Base] {};
\node[group,fit=(EscapeNorth) (EscapeSouth),label=below:Escape] {};
...
\path[-latex] (Flat) edge (Integers)
	(Lift) edge (Integers);
\node[below=2cm of Enums] {Booleans};
\end{tikzpicture}
\end{center}
